...
class SongBotService
{
    ...
    public function getPageData($pageNumber)
    {
        ...
        if ($pageNumber <= 0 || $rowsAmount < $pageNumber) {
            ...
            $rowStart = ($pageNumber - 1) * 10 + 1;
            $rowEnd = $pageNumber * 10;
            $dbProcess = new MySqliDBProcess($this->db_host, $this->username, $this->password, $this->db_name);
            $dataSet = $dbProcess->query(
                "SELECT * FROM ( 
                    SELECT @rowNum := @rowNum+1 AS rowNum, `fld_trigger`,`fld_sentence` FROM tb_songbot, (SELECT@rowNum := 0) AS r
                 ) AS PageData
                 WHERE {$rowStart} <= rowNum AND rowNum<= {$rowEnd}"
            );
            echo "SELECT * FROM ( 
                    SELECT @rowNum := @rowNum+1 AS rowNum, `fld_trigger`,`fld_sentence` FROM tb_songbot, (SELECT@rowNum := 0) AS r
                 ) AS PageData
     ...
     * @param $cardNo
     */
    public function getTreasureCruiseData($cardNo)
    {

        $sql = "SELECT * FROM `tb_TC` WHERE ID ='{$cardNo}'";
        $dbProcess = new MySqliDBProcess($this->db_host, $this->username, $this->password, $this->db_name);
        return $dbProcess->query($sql);
